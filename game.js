var Snake = (function () {
  ...
  const INITIAL_PLAYER = { x: Math.floor(tileCount / 20), y: Math.floor(tileCount / 2) };
...
Snake.setup.fixedTail(false);
    ...
    const URL = "https://teachablemachine.withgoogle.com/models/_ATgzV9r7/";

    let recognizer;
    
    // Fungsi untuk memulai model suara
    ...
    async function init() {
        await createModel();
        const classLabels = recognizer.wordLabels(); // Mendapatkan label kelas dari model
    
        // Siapkan elemen untuk menampilkan hasil prediksi
        const labelContainer = document.getElementById("label-container");
        for (let i = 0; i < classLabels.length; i++) {
            labelContainer.appendChild(document.createElement("div"));
        }
    
        // Mendengarkan suara dan melakukan aksi berdasarkan prediksi
        recognizer.listen(result => {
            const scores = result.scores; // Probabilitas prediksi untuk setiap kelas
            const maxIndex = scores.indexOf(Math.max(...scores));
            const pred = classLabels[maxIndex];
    
            // Mengontrol arah ular berdasarkan prediksi suara
            switch (pred) {
                case "Up":
                    Snake.action('up');
                    break;
            ...
            // Update label container dengan prediksi
            for (let i = 0; i < classLabels.length; i++) {
                const classPrediction = `${classLabels[i]}: ${result.scores[i].toFixed(2)}`;
                labelContainer.childNodes[i].innerHTML = classPrediction;
            }
        }, {
            probabilityThreshold: 0.75, // Hanya deteksi jika probabilitas di atas ambang ini
            overlapFactor: 0.5 // Faktor overlap untuk pengenalan suara
        });
    }
    
    // Memulai permainan
    Snake.start(5);
    Snake.setup.keyboard(false); // Nonaktifkan kontrol keyboard
    init();
